...
# ============================================================
# Field amplitude when the motor is being actuated / when idle (holding)
FIELD_AMP_MOTOR1_ACTIVE = 0.015;  FIELD_AMP_MOTOR1_IDLE = 0.015   # T
FIELD_AMP_MOTOR2_ACTIVE = 0.015;  FIELD_AMP_MOTOR2_IDLE = 0.015   # T
FIELD_AMP_MOTOR3_ACTIVE = 0.015;  FIELD_AMP_MOTOR3_IDLE = 0.015   # T
ROTATION_FREQ      = 2                                    # Hz
...
# ============================================================
def build_field_matrix(motor_positions):
    """
    6 x NUM_COILS actuation matrix  A  so that  b_target = A @ currents.
    Rows: [Bx1, By1, Bx2, By2, Bx3, By3].
    Each column is one coil's (Bx, By) contribution per unit current
    at the three motor positions, via the magnetic-dipole model.
    """
    A = np.zeros((6, NUM_COILS))
    for j, params in enumerate(COIL_PARAMS):
        m  = params[:3]
        r0 = params[3:]
        for i, (px, py, pz) in enumerate(motor_positions):
            ...
            r5 = r2 * r3
            mdot = m @ d
            A[2*i,     j] = _MU0_4PI * (3 * d[0] * mdot / r5 - m[0] / r3)
            A[2*i + 1, j] = _MU0_4PI * (3 * d[1] * mdot / r5 - m[1] / r3)
    return A
...
class ClosedLoopController:
    ...
    # ---- Core: compute B-field targets -> solve currents -> CAN -------
    def _send_field_step(self):
        ...
        ga1 = self.motor_phase[1]
        ga2 = self.motor_phase[2] + th1
        ga3 = self.motor_phase[3] + th2

        am = self.active_motor
        a1 = FIELD_AMP_MOTOR1_ACTIVE if am == 1 else FIELD_AMP_MOTOR1_IDLE
        a2 = FIELD_AMP_MOTOR2_ACTIVE if am == 2 else FIELD_AMP_MOTOR2_IDLE
        a3 = FIELD_AMP_MOTOR3_ACTIVE if am == 3 else FIELD_AMP_MOTOR3_IDLE

        b_target = np.array([
            a1 * np.cos(ga1), a1 * np.sin(ga1),
            a2 * np.cos(ga2), a2 * np.sin(ga2),
            a3 * np.cos(ga3), a3 * np.sin(ga3),
        ])

        A = build_field_matrix([
