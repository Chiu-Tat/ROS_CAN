...
class CANApp(QtWidgets.QMainWindow):
    ...
    def online_check(self):
        """Handle keyboard input for sequential coil current setting."""
        # Initialize the state variables
        self.current_coil = 1
        self.current_index = 0
        self.current_values_list = [2000, 6000, 10000, 2000, 6000, 10000, 2000, 6000, 10000, 2000, 6000, 10000, 2000, 6000, 10000]  # Current in mA
        self.space_count = 0
        ...
        class KeyPressFilter(QtCore.QObject):
            ...
            def eventFilter(self, obj, event):
                if event.type() == QtCore.QEvent.KeyPress and event.key() == QtCore.Qt.Key_Space:
                    parent = self.parent()
                    if parent.calibration_active:
                        # Set all device current values to 0
                        parent.send_can_message(0x003, [0x13, 0x88, 0x00, 0x00, 0x55, 0x55, 0x55, 0x55])
                        
                        # Delay 1ms
                        rospy.sleep(0.001)

                        # Calculate current CAN ID based on coil number
                        can_id = 0x013 + ((parent.current_coil - 1) * 0x010)
                        current = parent.current_values_list[parent.current_index]

                        # Set the current for the current coil
                        [data_0, data_1] = current_to_can(current)

                        data = [0x13, 0x88, data_0, data_1, 0x55, 0x55, 0x55, 0x55]
                        parent.send_can_message(can_id, data)
                        
                        # Print status
                        print(f"Setting Coil {parent.current_coil} to {current/1000.0}A")
                        
                        # Update counters
                        parent.space_count += 1
                        parent.current_coil += 1
                        
                        # Check if we need to move to next current value
                        if parent.current_coil > 10:
                            parent.current_coil = 1
                            parent.current_index += 1
                            
                        # Check if we're done with all combinations
                        if parent.space_count >= 150:
                            print("Calibration complete!")
                            parent.calibration_active = False
                            # Remove the event filter
                            parent.removeEventFilter(parent.key_filter)
        ...
        # Set focus to the main window to capture keyboard events
        self.setFocus()
        print("Online check started. Press SPACE to set coil currents sequentially.")
    ...
    def stop_devices(self):
        """Send a CAN message to stop all devices."""
        can_id = 0x003  # CAN ID
