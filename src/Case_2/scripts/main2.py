...
class CANApp(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        rospy.loginfo("Subscribed to sphere position topics: /sphere/red, /sphere/blue")
        
        # Store latest sphere positions
        self.latest_sphere_positions = {
            'red': {'x': None, 'y': None, 'time': None},
            'green': {'x': None, 'y': None, 'time': None}, # Keep structure but unused
    ...
    def online_check(self):
        """Start closed-loop trajectory control for magnetic objects (2 magnets)."""
        ...
        if self.closed_loop_controller is None:
            rospy.loginfo("Initializing closed-loop trajectory controller for 2 magnets...")
        ...
        try:
            ...
            if self.use_csv_trajectory:
                trajectory_file_path = "/home/dz/Documents/CUHK/ROS_CAN/src/Case_2/scripts/trajectory_case2_3D_sampled.csv"
                if not os.path.exists(trajectory_file_path):
                    rospy.logerr(f"CSV Trajectory file not found: {trajectory_file_path}")
                    return
                self.closed_loop_controller.prepare_trajectory(trajectory_file_path, use_csv=True)
            else:
                trajectory_file_path = "/home/dz/Documents/CUHK/ROS_CAN/src/Case_2/scripts/trajectory_case2_2D.json"
                if not os.path.exists(trajectory_file_path):
                    rospy.logerr(f"JSON Trajectory file not found: {trajectory_file_path}")
                    return
                self.closed_loop_controller.prepare_trajectory(trajectory_file_path, use_csv=False)
            ...
            rospy.loginfo(f"Closed-loop trajectory control started (Red, Blue) using {'CSV' if self.use_csv_trajectory else 'JSON'} trajectory")
